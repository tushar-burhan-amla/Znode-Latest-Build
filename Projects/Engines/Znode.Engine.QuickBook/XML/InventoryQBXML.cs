...
namespace Znode.Engine.QuickBook
{
    public class InventoryQBXML : IDisposable
    {
        ...
        => HelperUtility.GetQuickBookXML<ItemInventoryAddRq>
        (itemInventoryAddRq, QuickBookConstants.ItemInventoryAddRqXMLAttribute);
        ...
        /// <summary>
        /// Implementation ofIDisposible interface
        /// </summary>
        ...
        /// <summary>
        /// Mapping of sales order line item used in QuickBook response XML string generation from znode order line item model for creating new order if it's not available in QuickBook
        /// </summary>
        /// <param name="orderLineItem">znode order line item model</param>
        /// <returns>Sales order line item used in QuickBook response XML string</returns>
        ...
        => new SalesOrderLineAdd()
        {
            ...
            Desc = QuickBookConstants.DefaultNAText,
        ...
        /// <returns>Instance of mapped QuickBook ItemInventoryAdd Request object</returns>
        ...
        => new ItemInventoryAddRq()
        {
            ...
            ItemInventoryAdd = new ItemInventoryAdd()
            {
                ...
                SalesDesc = QuickBookConstants.DefaultNAText,
                SalesPrice = orderLineItemModel.Price.ToString("F")
            }
        };

        /// <summary>
        /// Creates instance for mapping QuickBook item reference for order line item with item inventory.
        /// </summary>
        /// <param name="orderLineItemModel">ZNode order line item model from which reference mapping needs to be done</param>
        /// <returns>Mapping for QuickBook item reference for order line item with item inventory.</returns>
        ...
        /// <summary>
        /// Gives reference model for default asset account type which is already available in QuickBook.
        /// </summary>
        /// <returns>Reference model for default asset account type which is already available in QuickBook.</returns>
        private Ref GetDefaultAssetAccountRef()
        => new Ref()
        {
            FullName = QuickBookConstants.AssetAccountType
        };

        /// <summary>
        /// Gives reference model for default income account type which is already available in QuickBook.
        /// </summary>
        /// <returns>Reference model for default income account type which is already available in QuickBook.</returns>
        private Ref GetDefaultIncomeAccountRef()
        => new Ref()
        {
            FullName = QuickBookConstants.IncomeAccountType
        };

        /// <summary>
        /// Gives reference model for default COGS account type which is already available in QuickBook.
        /// </summary>
        /// <returns>Reference model for default asset account type which is already available in QuickBook.</returns>
        private Ref GetDefaultCOGSAccountRef()
        => new Ref()
        {
            FullName = QuickBookConstants.COGSAccountType
        };
