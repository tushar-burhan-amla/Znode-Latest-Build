...
namespace Znode.Libraries.Hangfire.UserRegistrationAttempt
{
    public class UserRegistrationAttemptHelper : BaseScheduler, ISchedulerProviders
    {
        #region Schedular parameter
        protected override string SchedulerName => "ClearUserRegistrationAttempt";
        ...
        public void InvokeMethod(ERPTaskSchedulerModel model)
        {
            if (!string.IsNullOrEmpty(model.ExeParameters))
            {
                ...
                CallUserRegistrationAttemptAPI(args[1], args[4]);
            }
        }

        //To call remove user registration attempt details API.
        protected virtual void CallUserRegistrationAttemptAPI(string apiDomainURL, string token)
        {
            string requestPath = $"{apiDomainURL}/user/removealluserregistrationattemptdetails";
            ...
            try
            {
                ...
                catch (WebException webException)
                {
                    if (CheckTokenIsInvalid(webException))
                    {
                        ...
                        CallUserRegistrationAttemptAPI(apiDomainURL, token);
                    }
                    else
