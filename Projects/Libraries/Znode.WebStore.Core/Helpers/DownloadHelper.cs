...
namespace Znode.WebStore.Helpers
{
    public class DownloadHelper : IDownloadHelper
    {
        ...
        public void ExportDownload<T>(List<T> list, string fileType, HttpResponseBase response, string delimiter = null, string fileName = null, bool hasControl = false, bool displayColumn = true)
        {
            ...
            if (!Equals(list, null))
            {
                ...
                source = null;
            CallSpecificDownloadOption(fileType, source, response, delimiter, fileName, hasControl, displayColumn);
        }
        ...
        private void CallSpecificDownloadOption(string fileType, DataSet source, HttpResponseBase response, string delimiter, string fileName, bool hasControl = false, bool displayColumn = true)
        {
            if (!Equals(source, null))
            {
                switch (fileType)
                {
                    case "1":
                        ...
                        gridView.DataBind();
                        this.ExportDataToExcel(fileName, gridView, response, hasControl);
                        break;
                    case "2":
                        string strData = ExportToCSV(source, displayColumn);
        ...
        private string ExportToCSV(DataSet dataset, bool displayColumn = true)
        {
