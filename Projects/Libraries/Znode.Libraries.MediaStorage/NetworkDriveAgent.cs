...
{
    public class NetworkDriveAgent : IConnector
    {
        public NetworkDriveAgent(FileUploadPolicyModel policymodel)
        {
        ...
        public string Upload(MemoryStream stream, string fileName, string folderName)
        {
            ...
            string path = $"{UploadPolicyModel.NetworkDrivePath}/{UploadPolicyModel.BucketName}/{folderName}/{fileName}";
            //write to file
            ...
            return $"{UploadPolicyModel.NetworkDrivePath}/{UploadPolicyModel.BucketName}/{Path.GetFileName(fileName)}";
        }
        ...
        public List<string> Delete(string fileName, string folderName)
        {
            ...
            if (!string.IsNullOrEmpty(fileName))
            {
                foreach (string _fileName in fileName.Split(','))
                {
                    ...
                    //Retrive physical path of file 
                    string filePath = Path.Combine(UploadPolicyModel.NetworkDrivePath + $"{ serverPath}", _fileName);
