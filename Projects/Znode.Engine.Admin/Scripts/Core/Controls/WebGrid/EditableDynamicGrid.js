...
function DgUpdateString(t) {
    var e = "",
        ...
        n = {},
        a = !0,
        d = decodeURIComponent($(t).data("parameter")),
    ...
    return r.detail = {
        data: e,
        linkId: t
    }, a ? ($(document).trigger(r), DgCallAjax(e, t, function (result) {
        ZnodeNotification.prototype.DisplayNotificationMessagesHelper(result.message, result.status ? "success" : "error", isFadeOut, fadeOutTime)
        $(document).trigger("UpdateGrid");
        validateForm = result.status;
    }), e) : a
}
...
function DgUpdateSuccess(t) {
    ...
    $(e).find("[data-dgview='edit']").each(function (t, e) {
        "dropDown" == $(this).attr("class") ? SetBooleanTypeDropDownValue(this) : $(this).prev("[data-dgview='show']").text($(this).val())
...
function SetBooleanTypeDropDownValue(control) {
    if ("Boolean" == $(control).attr('data-columntype'))
        $(control).prev("[data-dgview='show']").html($(control).children("option:selected").text() == "True" ? "<i class='z-active'></i>" : "<i class='z-inactive'></i>");
    else
        $(control).prev("[data-dgview='show']").html($(control).children("option:selected").text());
}
var rowCount = 0,
    EditableGridEvent = {
        Init: function () {
            $(document).on("click", "#btnAddRow", function () {
                ...
                var i = $(this).data("parameter"),
                    n = $(this).attr("href"),
                    a = '<li><a oncontextmenu="return false" class="z-update z-ok" href="' + n + '" data-parameter="' + i + '" data-managelink="Update" title="Update"></a></li><li><a oncontextmenu="return false" class="z-cancel z-close" href="' + n + '" data-parameter="' + i + '" data-managelink="Cancel" title="Cancel"></a></li>';
                ...
                e.find("td").each(function () {
                    if ($(this).find('label').text() != undefined && $(this).find('label').text() != "" && $(this).find('input').val() != $(this).find("label").text()) {
