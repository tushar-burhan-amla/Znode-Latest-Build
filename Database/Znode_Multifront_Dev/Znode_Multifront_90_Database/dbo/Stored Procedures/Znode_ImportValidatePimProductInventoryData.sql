...
CREATE PROCEDURE dbo.Znode_ImportValidatePimProductInventoryData(
...
/*
    Summary :  Import Pim Product Inventory Data 
    Process :  Admin site will upload excel / csv file in database and create global temporary table
			...
			To import data call procedure "Znode_ImportPriceList"
...
*/
     BEGIN
         BEGIN TRY
		    ...
		    if NOT Exists (SELECT top 1 1 FROM @AttributeDetail where  iSNULL(SourceColumnName,'') = ''  and IsRequired = 1)
			 EXEC Znode_ImportInventory_Ver1 @TableName = @TableName , @Status = @Status , @UserId = @UserId , @ImportProcessLogId = @ImportProcessLogId , @NewGUID = @NewGUID;
		  -- Retrun record set 
		  ...
		  If Not Exists (Select top 1  1 FROM ZnodeImportLog where Guid = @NewGUID)
		  Begin
		    --Update process with completed status for current import 
			  Update ZnodeImportProcessLog SET Status = dbo.Fn_GetImportStatus (3), ProcessCompletedDate = GETUTCDATE() where  ImportProcessLogId = @ImportProcessLogId
		  END
             
	    END TRY
         BEGIN CATCH
             SELECT ERROR_MESSAGE() , ERROR_LINE() , ERROR_PROCEDURE();
             EXEC Znode_ImportReadErrorLog @ImportHeadName = 'Inventory' , @ImportProcessLogId = @ImportProcessLogId , @NewGUID = @NewGUID;
		   Update ZnodeImportProcessLog SET Status = dbo.Fn_GetImportStatus (3), ProcessCompletedDate = GETUTCDATE() where  ImportProcessLogId = @ImportProcessLogId
         END CATCH;
     END;