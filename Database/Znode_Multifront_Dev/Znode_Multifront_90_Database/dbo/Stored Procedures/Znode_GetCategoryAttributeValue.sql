CREATE PROCEDURE [dbo].[Znode_GetCategoryAttributeValue]
  ...
  */
     BEGIN
         BEGIN TRY
             ...
             WITH Cte_AttributeValueLocale
                  AS (SELECT ZPCAV.PimCategoryAttributeValueId,PimCategoryId,ZPCAVL.CategoryValue,AttributeCode,ZPA.PimAttributeId,ZPCAVL.LocaleId
                      ...
                      (
                          SELECT TOP 1 1
                          FROM dbo.split(@PimCategoryId, ',') SP
                          WHERE ZPCAV.PimCategoryId = SP.Item
                      )
                           AND EXISTS
                      (
                          SELECT TOP 1 1
         ...
         BEGIN CATCH
              DECLARE @Status BIT ;
		     SET @Status = 0;
		     DECLARE @Error_procedure VARCHAR(1000)= ERROR_PROCEDURE(), @ErrorMessage NVARCHAR(MAX)= ERROR_MESSAGE(), @ErrorLine VARCHAR(100)= ERROR_LINE(), @ErrorCall NVARCHAR(MAX)= 'EXEC Znode_GetCategoryAttributeValue @PimCategoryId = '+@PimCategoryId+',@PimAttributeId='+@PimAttributeId+',@LocaleId='+CAST(@LocaleId AS VARCHAR(10))+',@Status='+CAST(@Status AS VARCHAR(10));
