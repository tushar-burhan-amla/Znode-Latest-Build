CREATE PROCEDURE [dbo].[Znode_GetPimAttributesDetails]
    ...
    */ 
	 ...
	 BEGIN
         ...
         BEGIN TRY
             ...
             WITH Cte_PimAttribute
                  ...
                  AS (SELECT PimAttributeId,ParentPimAttributeId,AttributeTypeId,AttributeCode,IsRequired,IsLocalizable,IsFilterable,IsSystemDefined,IsConfigurable,
                      ...
                      WHERE EXISTS(SELECT TOP 1 1 FROM dbo.Split(@PimAttributeId, ',') SP WHERE SP.Item = CTPADV.PimAttributeId))
         ...
         BEGIN CATCH
             DECLARE @Status BIT ;
		     SET @Status = 0;
		     DECLARE @Error_procedure VARCHAR(1000)= ERROR_PROCEDURE(), @ErrorMessage NVARCHAR(MAX)= ERROR_MESSAGE(), @ErrorLine VARCHAR(100)= ERROR_LINE(), @ErrorCall NVARCHAR(MAX)= 'EXEC Znode_GetPimAttributesDetails @PimAttributeId = '+@PimAttributeId+',@LocaleId='+CAST(@LocaleId AS VARCHAR(50))+',@Status='+CAST(@Status AS VARCHAR(10));
