...
GO
CREATE NONCLUSTERED INDEX [IX_ZnodePrice_SKU]
    ON [dbo].[ZnodePrice]([SKU] ASC)
    INCLUDE([PriceListId], [SalesPrice], [RetailPrice], [UomId], [UnitSize], [ActivationDate], [ExpirationDate], [ExternalId]);


GO
CREATE NONCLUSTERED INDEX [IX_ZnodePrice_PriceListId_SKU]
    ON [dbo].[ZnodePrice]([PriceListId] ASC, [SKU] ASC)
    INCLUDE([SalesPrice], [RetailPrice], [UomId], [UnitSize], [ActivationDate], [ExpirationDate], [ExternalId]);
